...
1. **Startup and loading speed**
   ...
   - Pixel 10 / PowerVR reports are a specific renderer/engine investigation, not a generic low-performance category. `v0.2.399` includes the all-PowerVR cache workaround and Auto/Vulkan/OpenGL selection. Record the selected mode, GPU, active renderer, whether `NMainMenu` appeared, and whether 1s/3s/10s/30s/60s post-startup markers were written. Attach `last_renderer_attempt.txt` and `last_process_exit_info.txt` when present.
...
| --- | --- | --- | --- |
| Physical ARM64 Samsung test device | `arm64-v8a` | Exact `v0.2.399` update install; Auto/Vulkan and explicit OpenGL public launch to real `NMainMenu`; 120-second heartbeat; public BaseLib/Quick Restart/SavesMerger activation evidence; actual Quick Restart room restore; earlier Steam Cloud Pull and guarded Push behavior | Working in connected validation; final PowerVR and full physical viewport coverage pending |
...
- Renderer/driver compatibility must be validated per device. `v0.2.399` replaces forced OpenGL with explicit Auto/Vulkan/OpenGL selection and includes the PowerVR cache workaround, but it still needs reporter-class PowerVR proof.
