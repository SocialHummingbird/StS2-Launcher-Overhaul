...
1. **Startup and loading speed**
   ...
   - Pixel 10 / PowerVR reports are a specific renderer/engine investigation, not a generic low-performance category. Current unreleased source backports the all-PowerVR cache workaround and adds Auto/Vulkan/OpenGL selection. Record the selected mode, GPU, active renderer, whether `NMainMenu` appeared, and whether 1s/3s/10s/30s/60s post-startup markers were written. Attach `last_renderer_attempt.txt` and `last_process_exit_info.txt` when present.
...
- Renderer/driver compatibility must be validated per device. The latest published APK can force OpenGL Compatibility and issue #34 shows that path failing on a modern PowerVR device; unreleased source replaces it with explicit Auto/Vulkan/OpenGL selection and still needs reporter-class proof.
