...
- Compact launcher branch-availability failure text now also treats `passwordRequired=true` marker metadata as `password-protected` for the selected branch and visible branch list, so a password-protected branch with exposed Windows manifests is not described as downloadable in failure/status guidance.
