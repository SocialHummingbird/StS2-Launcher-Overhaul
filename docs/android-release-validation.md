...
3. Confirm the compatibility step reports the same package name as the installed release package.
4. Confirm the compatibility step reports the same signer SHA-256 as `ANDROID_RELEASE_SIGNER_SHA256`.
5. Confirm the compatibility step reports a higher `versionCode` than the baseline APK.
6. Do not announce the APK as update-compatible if the run used `allow_update_baseline_reset=true`; that mode creates a new stable baseline and cannot update APKs signed by a different previous key.
