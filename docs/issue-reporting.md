...
For crashes after the real main menu appears, include `last_post_startup_trace.txt`, `last_post_startup_heartbeat.txt`, `last_app_lifecycle_event.txt`, `last_renderer_attempt.txt`, and `last_process_exit_info.txt` or other Android tombstone evidence when available. Issue #34 has exact Pixel 10 Pro / Android 17 / PowerVR D-Series DXT-48-1536 evidence: `v0.2.397` reached `NMainMenu` and then exited, while `v0.2.399` reaches the game in all modes and isolates the remaining defect to missing touch under Auto/Vulkan/Safe. OpenGL touch works, with temporary cold-menu slowdown. Current unpublished source also records `graphics_device.txt` and forces OpenGL on detected PowerVR devices. For similar reports, distinguish process death from a live but nonresponsive renderer, and record requested/effective mode, touch behavior, first-menu timing, later-menu timing, and post-startup markers.
