...
For crashes after the real main menu appears, include `last_post_startup_trace.txt`, `last_post_startup_heartbeat.txt`, `last_app_lifecycle_event.txt`, `last_renderer_attempt.txt`, and `last_process_exit_info.txt` or other Android tombstone evidence when available. Issue #34 established the useful distinction: the Pixel 10 Pro / Android 17 / PowerVR process first exited after `NMainMenu`, then a later build reached the game without touch under Vulkan, while OpenGL worked. The reporter subsequently confirmed that the automatic PowerVR/OpenGL route loads the game with active touch. For similar reports, distinguish process death from a live but nonresponsive renderer, and record exact APK, requested/effective mode, `graphics_device.txt`, touch behavior, first-menu timing, later-menu timing, and post-startup markers.
