...
Each launch atomically replaces `last_mod_launch.json`. It records only the launch mode, exact selection fingerprint, timestamp, discovered/loaded/active/partial/failed counts, and one short result per selected mod. During a live load, a missing runtime state is Failed, never Loaded. If the marker itself is missing, the later launcher UI says `Not tested yet`. The launcher does not merge, copy, or reinterpret vanilla and modded saves.

The Mods page reads the persisted selection and discovered files as soon as it opens, even when sign-in, download, or mod loading is unavailable. `Vanilla` and `Modded` are selectors; Home then shows `Play Vanilla` or `Play Modded · N enabled` and the next save namespace. Installed and Enabled for Modded describe the current local choice. `Loaded last launch`, `Partial`, and `Failed` come only from a matching, internally consistent `last_mod_launch.json`; a missing, corrupt, contradictory, or different-fingerprint result is `Not tested yet`. The Saves page states that vanilla and modded save sets sync separately and are not merged. Clearing staged Workshop files requires confirmation.
