...
Workshop/mod support is in progress. The current latest APK is `v0.2.401-native-modded-save-pull`. It preserves the v0.2.400 PowerVR touch-compatible renderer routing, dedicated Mods destination, strict public/default per-mod activation evidence, and connected behavioral Quick Restart proof while adding native modded-save restoration after Manual Pull. The current Android Workshop path can discover, stage, present, and launch the tested mod set:
