...
| Steam Cloud safety | Current Stage 2 uses one verified Pull/Upload transfer with explicit vanilla/modded allowlists, exact SteamID64/runtime/mod-set context, immutable snapshots, destination backups, tombstones, failure propagation, and destination read-back hashes. Upload eligibility depends only on transferable local saves and no interrupted Pull marker. Older fix28-fix31 artifacts describe the superseded Pull/save-origin gate and do not validate the current implementation. | Requires release-candidate evidence for direct Upload without a preceding Pull or Steam installation, interrupted-Pull blocking, all context mismatches, backup/tombstone behavior, commit and connection failures, read-back mismatch, and successful Pull/Upload. |
...
9. Save and Steam Cloud safety.
   ...
   Evidence must show exact local allowlist eligibility, interrupted-Pull blocking, context isolation across account/namespace/runtime/mod set, destination backup before overwrite, deletion/tombstone behavior, failure propagation, verified read-back, and no false `synced` result. Pull is not an Upload prerequisite.
...
- Local fix31 `0.2.188-local-runtime-beta-fix31-save-origin-pck` / `versionCode=218858` historically fixed a false save-origin mismatch in the superseded gate. Its evidence remains useful for runtime routing, but it does not prove current Stage 2 transfer safety; no Steam Cloud Upload was performed.
...
- Steam Cloud Upload remains blocked from release signoff until the current one-operation transfer is exercised for direct eligibility, interrupted Pull, every context mismatch, destination backup/tombstones, failure propagation, read-back mismatch, and controlled success. Local fix28 evidence proves only the superseded selected-version gate and is retained as historical evidence.
