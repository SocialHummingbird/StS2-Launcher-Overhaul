...
| Cache cleanup | Selected-version redownload and inactive cache cleanup are branch-aware by design, and inactive cleanup removes stale runtime packs while preserving the selected runtime pack when present. | Requires evidence that selected cache/runtime pack is preserved or cleared only when intended. |
