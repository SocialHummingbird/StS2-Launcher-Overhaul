...
| --- | --- | --- |
| Public/default branch | Public/default remains available and keeps the legacy `files/game` install path. Local fix27 ARM64 evidence proves public/default launcher startup and game main-menu launch with matched PCK/runtime hashes. Local fix29 ARM64 evidence proves the current package opens the public/default launcher without immediate crash, validates the public PCK, writes current runtime-slot evidence, and reports playable through the legacy packaged public runtime path. | Requires release-candidate repeat evidence for public/default download or update check, launch, and cloud Pull posture. |
...
- Local fix29 ARM64 evidence at `artifacts/android/fix29-readonly-current-marker-status-20260619-115426` confirms the current installed package `0.2.188-local-runtime-beta-fix29-diagnostics` / `versionCode=218856` opens the public/default launcher without immediate crash, validates the public PCK, writes current runtime-slot evidence, and reports public playable through the legacy packaged runtime path. The public runtime-pack compatibility manifest was not installed in this path, so this is public launcher/runtime sanity evidence only, not proof that public-beta gameplay or matched beta managed runtime launch currently passes.
