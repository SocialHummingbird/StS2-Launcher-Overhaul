...
- Vanilla and native modded saves remain in separate namespaces. Current Pull and automatic sync use only the selected exact-context allowlist and never seed a missing modded namespace from vanilla data.
- Current unreleased source provides desktop-tested read-only legacy-save discovery, locally hashed support export, context-separated/quarantined snapshots, and byte-for-byte local Restore/Undo. Recovery cannot contact Steam and unknown provenance cannot be approved or synchronized. None of this is a public fix until the complete Stage 5 physical matrix passes.
...
- Exact-build ARM64 validation of legacy scan/export, local Restore/Undo, interrupted recovery, and explicit post-validation approval remains pending. Desktop fake stores prove the non-network recovery contract; they do not prove Android storage behavior or real Steam reconciliation.
