...
internal readonly struct LaunchAttemptSummary
{
    ...
    internal string RecoveryHint()
    {
        ...
        if (ContainsText(Phase, LauncherLaunchAttemptPhases.Checking))
            return "Suggested next step: Start Game stopped while checking the selected version. Try Start Game again, then use Help Report if it repeats.";
        ...
        if (ContainsText(Phase, LauncherLaunchAttemptPhases.SetupFailed))
            return HasConcrete(ProblemOrDetail())
                ...
                : "Suggested next step: reopen the launcher and use Help Report if launch setup fails again.";
        ...
        if (ContainsText(Phase, LauncherLaunchAttemptPhases.ModReadinessFailed))
            return "Suggested next step: switch to Play Vanilla once. If vanilla starts, re-enable mods one at a time and share a Help Report for the failing mod set.";
        ...
        if (ContainsText(Phase, LauncherLaunchAttemptPhases.LaunchHandoffNotRequested))
            return "Suggested next step: use Help Report before retrying. The launcher finished readiness but did not request Android to start the game.";
        ...
        if (ContainsText(Phase, LauncherLaunchAttemptPhases.LaunchHandoffFailed)
            ...
            return "Suggested next step: try Safe Start once. If it repeats, use Help Report because Android handoff failed after readiness passed.";

        if (ContainsText(Phase, LauncherLaunchAttemptPhases.SafeAndroidRestartRequested))
            return "Suggested next step: Safe Start was already requested. Use Help Report if the game still failed after this restart.";

        if (ContainsText(Phase, "restart") || ContainsText(Phase, "in-process"))
            return "Suggested next step: try Safe Start once, then use Help Report if startup still stalls.";

        return "Suggested next step: try Safe Start once, then use Last Problem or Help Report if startup still fails.";
    }
