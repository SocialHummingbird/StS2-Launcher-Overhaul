...
internal sealed partial class LauncherSessionCoordinator
{
    ...
    private void ShowSessionState(SessionState state)
    {
        ...
        switch (state)
        {
            case SessionState.Connecting:
                _view.SetStatus("Connecting to Steam...", LauncherStatusSeverity.Working);
                break;

            case SessionState.Authenticating:
                _view.SetStatus("Authenticating...", LauncherStatusSeverity.Working);
                break;

            case SessionState.VerifyingOwnership:
                _view.SetStatus("Verifying game ownership...", LauncherStatusSeverity.Working);
                break;
    ...
    private void ShowFailed()
    {
        ...
        _view.SetStatus(_model.FailureStatus(), LauncherStatusSeverity.Error);
