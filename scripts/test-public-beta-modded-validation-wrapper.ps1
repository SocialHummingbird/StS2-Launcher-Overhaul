...
$checks = [ordered]@{
    ...
    "requires activation marker v2" = 'predates per-mod activation evidence'
    "requires per-mod activation evidence" = 'activationEvidence'
    "requires Quick Restart Harmony targets" = 'HarmonyTargetCount'
    "classifies BaseLib partial compatibility" = 'partial-android-compatibility'
    "rejects launcher compatibility substitutes" = 'compatibilitySubstituteMods\s*-ne\s*0'
    "rejects failed mod runtime loads" = 'failedMods'
    "does not overclaim in-game verification" = 'inGameVerifiedMods'
    "requires BaseLib evidence" = '"BaseLib"'
    "requires Quick Restart evidence" = 'Quick\\s\*Restart\|QuickRestart'
    "rejects deprecated SavesMerger selection" = 'Deprecated SavesMerger still appears in selected runtime mods'
