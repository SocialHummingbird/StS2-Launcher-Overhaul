...
$sources = @(
    (Join-Path $root "src\STS2Mobile\Launcher\LauncherMarkerFile.cs"),
    (Join-Path $root "src\STS2Mobile\Launcher\LauncherMarkerFile.Read.cs"),
    (Join-Path $root "src\STS2Mobile\Launcher\LauncherMarkerFile.Typed.cs"),
    ...
    (Join-Path $root "src\STS2Mobile\Launcher\LauncherCloudSyncEvidence.Pull.cs"),
    (Join-Path $root "scripts\tests\stubs\LauncherCloudSafetyEvidenceStubs.cs"),
    (Join-Path $root "scripts\tests\LauncherCloudSafetyEvidenceTest.cs")
)

foreach ($source in $sources) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
        throw "Required cloud safety evidence test source is missing: $source"
    }
}
...
$output = [System.IO.Path]::GetFullPath(
    (Join-Path $tempRoot (
        "sts2-cloud-safety-evidence-" + [Guid]::NewGuid().ToString("N")
    ))
)
...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <AssemblyName>LauncherCloudSafetyEvidenceTest</AssemblyName>
  </PropertyGroup>
  <ItemGroup>
...
try {
    ...
    $projectPath = Join-Path $output "LauncherCloudSafetyEvidenceTest.csproj"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Launcher cloud safety evidence tests failed."
    }
} finally {
