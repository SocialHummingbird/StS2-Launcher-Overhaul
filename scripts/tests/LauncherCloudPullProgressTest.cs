...
internal static class LauncherCloudPullProgressTest
{
    ...
    private static int Main()
    {
        ...
        Run("all Pull phases are observable", AllPullPhasesAreObservable);
        Run("backup counts are cumulative", BackupCountsAreCumulative);
        Run("transfer counts and current path are visible", TransferCountsAreVisible);
        Run("profile preparation and seeding are distinct", ProfilePhasesAreDistinct);
        ...
        Run("partial completion exposes the unfinished count", PartialCompletionExposesCount);
        Run("failure remains visible with retained progress", FailureRetainsProgress);
        Run("terminal state ignores late operation updates", TerminalStateIgnoresLateUpdates);

        AssertEqual("Pull progress tests passed", 11, _passed);
        Console.WriteLine("Launcher cloud Pull progress tests passed 11/11.");
        return 0;
    }
    ...
    private static void AllPullPhasesAreObservable()
    {
        var snapshots = RunCompletePull();
    ...
    private static void TransferCountsAreVisible()
    {
        var snapshots = RunCompletePull();
        var transfer = snapshots.Last(
        ...
        AssertEqual("transfer completed", 2, transfer.TransferCompletedCount);
        AssertEqual("transfer skipped", 1, transfer.TransferSkippedCount);
        ...
        AssertContains("transfer downloaded count", presentation.DetailText, "2 downloaded");
        AssertContains("transfer skipped count", presentation.DetailText, "1 not in cloud");
        AssertContains("current transfer path", presentation.DetailText, "profile3/saves/prefs.save");
    }

    private static void ProfilePhasesAreDistinct()
    {
        var snapshots = RunCompletePull();
        var preparation = snapshots.Last(
            state => state.Phase == CloudOperationPhase.PreparingProfiles
        );
        var seeding = snapshots.Last(
            state => state.Phase == CloudOperationPhase.SeedingProfiles
        );

        AssertEqual(
            "profile preparation processed",
            2,
            preparation.ProfilePreparationProcessedCount
        );
        AssertEqual(
            "profile preparation private backups",
            1,
            preparation.PrivateBackupCreatedCount
        );
        AssertEqual("profile seed processed", 2, seeding.ProfileSeedProcessedCount);
        AssertEqual("profiles seeded", 1, seeding.ProfileSeededCount);

        var seedPresentation = CloudOperationPresentation.Create(seeding);
        AssertContains("seed phase", seedPresentation.PhaseText, "modded profiles");
        AssertContains("seed count", seedPresentation.DetailText, "1 seeded");
    }

    private static void PendingPhasesReadAsActive()
    {
        var pending = RunCompletePull()
        ...
        foreach (var state in pending)
        {
            ...
            AssertFalse(
                ...
                string.IsNullOrWhiteSpace(presentation.PhaseText)
            );
            AssertFalse(
                $"{state.Phase} detail text",
                string.IsNullOrWhiteSpace(presentation.DetailText)
            );
            AssertTrue(
                $"{state.Phase} progress above zero",
    ...
    private static void CurrentPathIsVisibleBeforeCompletion()
    {
        ...
        tracker.TransferPathStarted(
            "profile2/saves/history/2026-07-25.run"
        );

        var waiting = tracker.State;
        var presentation = CloudOperationPresentation.Create(waiting);
        AssertEqual(
            "waiting transfer processed",
            0,
            waiting.TransferProcessedCount
        );
        AssertContains(
        ...
        AssertTrue("waiting state visible", presentation.Visible);
    }
    ...
    private static void CompletionRemainsVisible()
    {
        ...
        AssertContains("complete downloaded", presentation.DetailText, "2 downloaded");
        AssertContains("complete seeded", presentation.DetailText, "1 modded file(s) seeded");
        AssertContains("complete finished", presentation.DetailText, "Pull is finished");
    }

    private static void PartialCompletionExposesCount()
    {
        var tracker = new CloudOperationProgressTracker(
            CloudOperationKind.Pull
        );
        ...
        tracker.PartiallyCompleted("Safety deadline reached");

        var presentation = CloudOperationPresentation.Create(tracker.State);
        AssertTrue("partial completion visible", presentation.Visible);
        AssertTrue("partial presentation", presentation.IsPartial);
        AssertEqual(
        ...
        AssertContains(
            "partial checked count",
            presentation.DetailText,
            "2/5 checked"
        );
        AssertContains(
            "partial downloaded count",
            presentation.DetailText,
            "1 downloaded"
        );
    }
    ...
    private static void FailureRetainsProgress()
    {
        ...
        tracker.EnumerationStarted("Finding paths");
        tracker.EnumerationCompleted(4);
        tracker.BackupStarted(4);
        ...
        var transferPresentation = CloudOperationPresentation.Create(
            tracker.State
        );
    ...
    private static void TerminalStateIgnoresLateUpdates()
    {
        ...
        AssertEqual(
            "late update snapshot count",
            terminalSnapshotCount,
            snapshots.Count
        );
        AssertEqual(
            "terminal phase preserved",
            CloudOperationPhase.Failed,
            tracker.State.Phase
        );
        AssertEqual(
            "late download excluded",
            0,
            tracker.State.TransferCompletedCount
        );
        AssertEqual(
            "terminal reason preserved",
            "request timed out",
            tracker.State.ErrorMessage
        );
    }
    ...
    private static IReadOnlyList<CloudOperationState> RunCompletePull()
    {
        ...
        tracker.ProfilePreparationStarted(2);
        ...
        tracker.TransferProcessed(
            "profile3/saves/prefs.save",
            completed: true,
            skipped: false
        );
        tracker.ProfileSeedingStarted(2);
        ...
        tracker.Finalizing("Refreshing backup mirror");
