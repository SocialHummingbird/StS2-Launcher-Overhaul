...
$sources = @(
    ...
    (Join-Path $root "scripts\tests\LauncherCloudUploadEligibilityTest.cs")
)

foreach ($source in $sources) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
        throw "Required cloud Upload eligibility test source is missing: $source"
    }
}
...
$output = [System.IO.Path]::GetFullPath(
    (Join-Path $tempRoot (
        "sts2-cloud-upload-eligibility-" + [Guid]::NewGuid().ToString("N")
    ))
)
...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <AssemblyName>LauncherCloudUploadEligibilityTest</AssemblyName>
  </PropertyGroup>
  <ItemGroup>
...
try {
    ...
    $projectPath = Join-Path $output "CloudUploadEligibilityTests.csproj"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Launcher cloud Upload eligibility tests failed."
    }
} finally {
