...
- Steam beta/version selection is implemented for validation, with a dropdown-first selector, `REFRESH GAME VERSIONS` Steam app-info discovery, concise branch options, selected-branch metadata/status helper text, branch-aware downloads, side-by-side non-public caches, selected-version diagnostics, `last_steam_branch_availability.txt` evidence, native routing/fallback diagnostics, branch-switch marker evidence, and safe branch-switch warnings. Static CI guardrails cover version-selection docs, release blockers, Autofill guardrails, and managed/native selector-guidance parity. It is not release-signed yet: beta password behavior, inaccessible/private branch handling, refresh/dropdown device behavior, startup routing, cache cleanup, Push backup evidence, Autofill provider behavior, and save compatibility across branches still require ARM64 device validation.
