...
- Latest published APK prerelease: `v0.2.292-local-audit-docs-split`
- Current APK asset: `StS2Launcher-v0.2.292-local-audit-docs-split-arm64-v8a.apk`
- Package name: `com.sts2launcher.overhaul.fork.local`
- Release asset SHA-256: `b12ccba83db235266580e41b7fc8f28a061ece115b107dc17d813099fa0f52c2`
- Latest local ARM64 build APK: `0.2.292-local-audit-docs-split` / `versionCode=292000`, artifact `artifacts/android/StS2Launcher-v0.2.292-local-audit-docs-split-arm64-v8a.apk`, SHA-256 `b12ccba83db235266580e41b7fc8f28a061ece115b107dc17d813099fa0f52c2`. This is a build/static-audit prerelease for the Steam version-selection audit module split covering root orchestration, action-section guardrails, helper-boundary guardrails, and focused docs/status guardrails; it does not replace the existing device evidence for public/public-beta runtime behavior. No Steam Cloud Push was run during this validation.
...
- Steam Workshop mod support has first-pass ARM64 public-branch evidence: subscribed Workshop items are discovered, usable depot-manifest items are staged into app-private storage, `BaseLib` and `Quick Restart` load in the Android runtime, and the main menu is reached without running Steam Cloud Push. One known subscribed item, `3747532120` / `Vanilla and Modded Saves Merger`, remains explicitly unsupported because Steam exposes only a legacy UGC handle with no direct URL or depot manifest. See [Android Steam Workshop mods](docs/android-workshop-mods.md).
...
- **Steam Workshop mods**  
  Subscribed Workshop mods can be synced into app-private Android storage and loaded by the runtime mod-loader patch. The first ARM64 public-branch evidence loads `BaseLib` and `Quick Restart` and reaches the main menu. Workshop sync and clear do not run Steam Cloud Push, and manual Push is locked while active staged Workshop PCK mods are present.
- **Mobile adaptation**  
...
.\scripts\verify-android-release-apk.ps1 `
  -ReleaseTag "v0.2.292-local-audit-docs-split" `
  -AssetName "StS2Launcher-v0.2.292-local-audit-docs-split-arm64-v8a.apk" `
  -Abi arm64-v8a
...
```text
Release: v0.2.292-local-audit-docs-split
